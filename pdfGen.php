...
const IMG = __DIR__ . '\graph\tmp_img';
require_once __DIR__.'/tfpdf.php';
require_once __DIR__.'/php/getresults.php';
require_once __DIR__.'/php/getPdfData.php';
require_once "drawCharts.php";
...
$pdf->Write(4, $pdf_data['site'], 'https://rimera.ru');
$pdf->SetTextColor($pdf_data['text_main'][0], $pdf_data['text_main'][1], $pdf_data['text_main'][2]);
...
$pdf->SetTextColor($pdf_data['text_secondary'][0], $pdf_data['text_secondary'][1], $pdf_data['text_secondary'][2]);
$pdf->Write(5,$pdf_data['pump_selected'].' - ' . (string)$pump['title']);
...
$pdf->SetFont('ArialBd','',13);
$pdf->Write(4, (string)$pump['frequency']);
$pdf->SetFont('ArialReg','',13);
...
$pdf->Write(4, 0);
$pdf->SetFont('ArialReg','',13);
$pdf->Write(4, ' ' . $pdf_data['unit_temp']);									//значок градуса, вопрос поддерживается ли при любых условиях
...
$pdf->Write(4, 50);
$pdf->SetFont('ArialReg','',13);
$pdf->Write(4, ' ' . $pdf_data['unit_freq'] . ' (');
$pdf->SetFont('ArialBd','',13);
$pdf->Write(4, 2910);
$pdf->SetFont('ArialReg','',13);
...
$pdf->Cell($width*200/1920, $height*47/1905,(string)$pump['title'],'LB',0,'C');
$pdf->SetFont('ArialReg','',13);
$pdf->Cell($width*120/1920, $height*47/1905, (string)$pump['amount_stage'],'B',0,'C');
$pdf->SetFont('ArialBd','',13);
$pdf->Cell($width_freq, $height*47/1905, (string)$pump['frequency'],'B',0,'C');
$pdf->Cell($width_speed, $height*47/1905, (string)$pump['rotation_speed'],'B',0,'C');
$pdf->Cell($width_power, $height*47/1905, (string)$pump['power_using'],'B',0,'C');
$pdf->SetFont('ArialReg','',13);
$pdf->Cell($width_eff, $height*47/1905, (string)$pump['efficiency'],'B',0,'C');
$pdf->Cell($width_pres, $height*47/1905, '0,1','B',0,'C');
$pdf->Cell($width_force, $height*47/1905, '3500','B',0,'C');
$pdf->Cell($width_length, $height*47/1905, (string)$pump['length'],'B',0,'C');
...
$pdf->Image(__DIR__ . '\img\logo1.png', $width*1670/1920, 10, 18);		//изображение для примера просто, когда будет известен размер, можно тоже выровнять по правой границе
$pdf->Image(__DIR__ . '\img\logoOutro.png', $width*1120/1920, 450 , 175);		//изображение для примера просто, когда будет известен размер, можно тоже выровнять по правой границе
...
$pdf->Output('I',(string)$pump['title'] . '.pdf', true);		//отдаём пользователю. вопрос стоит ли сохранять именно так, на русском
